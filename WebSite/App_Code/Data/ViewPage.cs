        ...
        public ViewPage ToResult(ControllerConfiguration configuration, XPathNavigator mainView)
        {
            ...
            else
            {
                ...
                if (IncludeMetadata("items"))
                {
                    ...
                    foreach (var field in Fields)
                        if (!ApplicationServicesBase.IsTouchClient || ((field.ItemsStyle == "CheckBoxList") || (!string.IsNullOrEmpty(field.ItemsTargetController) || viewIsForm)))
            ...
            if (!ApplicationServicesBase.Create().IsSystemController(Controller) && !AccessControlList.IsPermissionGranted(PermissionKind.Controller, Controller, "read"))
            {
                Rows.Clear();
                TotalRowCount = 0;
                Aggregates = null;
