...
namespace DatatecnixOfficerII.Data
{
    public class ConnectionStringSettingsFactoryBase
    {
        ...
        protected virtual ConnectionStringSettings CreateSettings(string connectionStringName)
        {
            if (string.IsNullOrEmpty(connectionStringName))
                connectionStringName = "DatatecnixOfficerII";
    ...
    public partial class SqlStatement : SqlStatementBase, IDisposable
    {
        ...
        public static void ValidateParameterValues(DbCommand command)
        {
            var paramTypeName = command.GetType().Name;
            if (paramTypeName.Contains("Oracle") || paramTypeName.Contains("Npgsql"))
                foreach (DbParameter p in command.Parameters)
                    ValidateParameterValue(p);
        }

        public static void ValidateParameterValue(DbParameter p)
        {
            var value = p.Value;
            if (!DBNull.Value.Equals(value) && (value != null))
            {
                var paramTypeName = p.GetType().Name;
                if ((value is Guid) && paramTypeName.Contains("Oracle"))
                    p.Value = ((Guid)(value)).ToByteArray();
                if ((value is DateTime) && paramTypeName.Contains("Npgsql"))
                {
        ...
        public virtual void AssignParameter(string name, object value)
        {
            ...
            {
                if (!Command.GetType().Name.Contains("Npgsql"))
                {
                    if ((bool)(value))
                        value = 1;
                    else
