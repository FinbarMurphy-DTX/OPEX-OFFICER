...
using DatatecnixOfficerII.Services;

namespace DatatecnixOfficerII.Data
{
    public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if ((res == null) || (res == DefaultDataControllerStream))
                    res = ControllerConfigurationUtility.GetResourceStream(string.Format("DatatecnixOfficerII.controllers.{0}.xml", controller), string.Format("DatatecnixOfficerII.{0}.xml", controller));
                if (res == null)
                {
        ...
        private void AddComputedExpressions(SelectClauseDictionary expressions, ViewPage page, CommandConfigurationType commandConfiguration, bool generateFormula)
        {
            ...
            foreach (var field in page.Fields)
            {
                var expressionName = field.ExpressionName();
                if (generateFormula && expressions.ContainsKey(expressionName))
                    expressions[("__original__" + expressionName)] = expressions[expressionName];
                if (!string.IsNullOrEmpty(field.Formula))
                {
                    if (useFormulaAsIs)
                        expressions[expressionName] = field.Formula;
                    else
                    {
                        if (generateFormula)
                        {
                            expressions[expressionName] = string.Format("({1}{0}{2})", field.Name, _leftQuote, _rightQuote);
                            expressions[("__original__" + expressionName)] = string.Format("({0})", field.Formula);
                        }
                        else
                            expressions[expressionName] = string.Format("({0})", field.Formula);
                    }
                }
                else
                {
                    ...
                    {
                        if (useFormulaAsIs)
                            expressions[expressionName] = field.Name;
                        else
                        {
                            var sqlExpr = string.Format("({1}{0}{2})", field.Name, _leftQuote, _rightQuote);
                            if (!string.IsNullOrEmpty(field.ItemsTargetController) || field.IsVirtual)
                                sqlExpr = "null";
                            expressions[expressionName] = sqlExpr;
                        }
                    }
                }
            }
        ...
        private bool ConfigureCommandForDelete(DbCommand command, ViewPage page, SelectClauseDictionary expressions, string tableName, FieldValue[] values)
        {
            ...
            command.CommandText = sb.ToString();
            SqlStatement.ValidateParameterValues(command);
            return true;
        }
        ...
        private bool ConfigureCommandForInsert(DbCommand command, ViewPage page, SelectClauseDictionary expressions, string tableName, FieldValue[] values)
        {
            ...
            foreach (var v in values)
            {
                ...
                if (IsFieldInsertable(field) && (v.Modified || field.HasDefaultValue))
                {
                    sb.AppendLine();
                    if (firstField)
                    {
        ...
        private void ConfigureCommandForSelect(DbCommand command, ViewPage page, SelectClauseDictionary expressions, string fromClause, string whereClause, string orderByClause, CommandConfigurationType commandConfiguration)
        {
            ...
            else
            {
                ...
                if (commandConfiguration == CommandConfigurationType.SelectDistinct)
                {
                    ...
                    if (distinctField.Type.StartsWith("Date"))
                    {
                        ...
                        if (commandType.Contains("MySql"))
                            distinctExpression = string.Format("cast({0} as date)", distinctExpression);
                        if (commandType.Contains("Oracle"))
                            distinctExpression = string.Format("trunc({0})", distinctExpression);
                        if (commandType.Contains("Npgsql"))
                            distinctExpression = string.Format("({0})::date", distinctExpression);
                    }
            ...
            var isResultSet = (!string.IsNullOrEmpty(fromClause) && fromClause.Contains(") resultset__"));
            if (!isResultSet)
            {
            ...
            if (((page.Filter != null) && (page.Filter.Length > 0)) || (isResultSet || !string.IsNullOrEmpty(_viewFilter)))
            {
                if (isResultSet)
                {
                    var resultsetExpressions = new SelectClauseDictionary();
                    foreach (var key in expressions.Keys)
                        if (!key.StartsWith("__original__"))
                        {
                            string expression = null;
                            if (!expressions.TryGetValue(("__original__" + key), out expression))
                                expression = expressions[key];
                            resultsetExpressions[key] = expression;
                        }
                    var saveFilter = page.Filter;
                    var saveHasWhere = _hasWhere;
                    var saveCurrentExpressions = _currentExpressions;
                    AppendSystemFilter(command, page, resultsetExpressions);
                    AppendAccessControlRules(command, page, resultsetExpressions);
                    AppendExternalFilter(command);
                    if (!string.IsNullOrEmpty(_viewFilter))
                    {
                        page.Filter = null;
                        var sqlSnapshot = sb.ToString();
                        _currentExpressions = resultsetExpressions;
                        AppendFilterExpressionsToWhere(sb, page, command, resultsetExpressions, whereClause);
                        var newSql = sb.ToString();
                        var filterBlock = newSql.Substring(sqlSnapshot.Length);
                        if (Regex.IsMatch(sqlSnapshot, "\\bwhere\\b", RegexOptions.IgnoreCase))
                        {
                            var whereMatch = Regex.Match(filterBlock, "\\bwhere\\b", RegexOptions.IgnoreCase);
                            if (whereMatch.Success)
                                filterBlock = ((filterBlock.Substring(0, whereMatch.Index) + " and ") + filterBlock.Substring((whereMatch.Index + whereMatch.Length)));
                        }
                        newSql = sqlSnapshot.Insert(sqlSnapshot.IndexOf(") resultset__"), filterBlock);
                        sb.Clear();
                        sb.Append(newSql);
                    }
                    page.Filter = saveFilter;
                    _hasWhere = saveHasWhere;
                    _viewFilter = null;
                    _currentExpressions = saveCurrentExpressions;
                }
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
                t = typeof(DatatecnixOfficerII.Data.Controller);
            var res = ControllerConfigurationUtility.GetResourceStream(string.Format("DatatecnixOfficerII.Reports.{0}", name), string.Format("DatatecnixOfficerII.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                var config = DatatecnixOfficerII.Data.Controller.CreateConfigurationInstance(t, controller);
