...
namespace DatatecnixOfficerII.Data
{
    ...
    public class BusinessRulesBase : ActionHandlerBase, DatatecnixOfficerII.Data.IRowHandler, DatatecnixOfficerII.Data.IDataFilter, DatatecnixOfficerII.Data.IDataFilter2
    {
        ...
        public virtual JObject UserClaims
        {
            get
            {
                if (_claims == null)
                {
                    _claims = ApplicationServicesBase.Create().OAuthReadUserClaims();
                    if (_claims == null)
                        _claims = new JObject();
                }
                return _claims;
            ...
            if (!IsSystemController(controllerName))
            {
                ...
                if (acl.Enabled)
                {
                    ...
                    while (lookupIterator.MoveNext())
                    {
                        ...
                        if (acl.PermissionGranted(PermissionKind.Controller, lookupController, "read"))
                        {
                            var newDataView = lookupIterator.Current.SelectSingleNode("@newDataView", Resolver);
                            if (((newDataView != null) && !string.IsNullOrEmpty(newDataView.Value)) && !acl.PermissionGranted(PermissionKind.Controller, lookupController, "create"))
                                newDataView.SetValue(string.Empty);
                        }
                        else
                        {
                            var lookupFieldName = lookupIterator.Current.Evaluate("string(parent::c:field/@name)", Resolver);
                            var lookupDataFieldIterator = Navigator.Select(string.Format("/c:dataController/c:views/c:view//c:dataField[@fieldName='{0}']", lookupFieldName), Resolver);
                            while (lookupDataFieldIterator.MoveNext())
                                if (string.IsNullOrEmpty(lookupDataFieldIterator.Current.GetAttribute("textMode", string.Empty)))
                                    lookupDataFieldIterator.Current.CreateAttribute(string.Empty, "textMode", string.Empty, "Static");
                                else
                                    lookupDataFieldIterator.Current.SelectSingleNode("@textMode").SetValue("Static");
                            lookupIterator.Current.DeleteSelf();
                        }
                    }
