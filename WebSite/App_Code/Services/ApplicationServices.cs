...
using DatatecnixOfficerII.Services.Rest;
...
namespace DatatecnixOfficerII.Services
{
    ...
    public class SiteContentFile
    {
        ...
        public static ActionResult Write(List<FieldValue> values)
        {
            ...
            var access = Controller.GrantFullAccess("opex_site_content");
            try
            {
        ...
        public static string ValidationKey
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(key) || key.Contains("AutoGenerate"))
                    key = "C78F60698D51F6E98D5F6AE12B11FDEC587374DB2EF5CCFDB72977BEAD37CAB695E90A23B79A2F18E5773ED9E553D282D62C418C6F29AFA37859648A479D56FD";
                return key;
            }
        }
    ...
    public partial class ApplicationServicesBase
    {
                ...
                if (_defaultSettings == null)
                {
                    ...
                    if (_defaultSettings == null)
                    {
                        ...
                        if (_defaultSettings == null)
                        {
                            ...
                            EnsureJsonProperty(_defaultSettings, "help.enabled", false);
        ...
        {
            DatatecnixOfficerII.Services.ApplicationServices.Initialize();
        }
        ...
        public virtual string SiteContentFieldName(SiteContentFields field)
        {
            ...
            if (field == SiteContentFields.CreatedDate)
                return "created_date";
            ...
            if (field == SiteContentFields.Length)
                return "length";
        ...
        public virtual SiteContentFileList ReadSiteContent(string relativePath, string fileName, int maxCount, DateTime? modified)
        {
            ...
            {
                if (modified.Value.Kind == DateTimeKind.Utc)
                    modified = modified.Value.ToLocalTime();
            ...
            foreach (var blobKey in blobsToResolve.Keys)
            {
                ...
                try
                {
                    access = Controller.GrantFullAccess("opex_site_content");
                    try
                    {
...
<body class=""studio Wide"" data-authorize-roles=""?"">
    <div id=""studio"" data-app-role=""page"">
        ...
        public virtual JObject UserSettings(Page p)
        {
            ...
            if (IsSiteContentEnabled)
            {
                var idProviders = ((JObject)(HttpContext.Current.Cache["sys/saas/%"]));
                if (idProviders == null)
                {
                    idProviders = new JObject();
                    foreach (var idProvider in ReadSiteContent("sys/saas", "%"))
                    {
                        var idProviderData = TextUtility.ParseYamlOrJson(idProvider.Text);
                        var displayName = Convert.ToString(idProviderData["Display Name"]);
                        if (!string.IsNullOrEmpty(displayName))
                            idProviders[idProvider.Name] = displayName;
                    }
                    HttpContext.Current.Cache.Add("sys/saas/%", idProviders, null, DateTime.Now.AddMinutes(15), Cache.NoSlidingExpiration, CacheItemPriority.Normal, null);
                }
                settings["idP"] = idProviders;
            }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            if (AuthorizationIsSupported)
            {
                ...
                try
                {
                    ...
                    if (css == null)
                        css = ConfigurationManager.ConnectionStrings["DatatecnixOfficerII"];
                ...
                if (Membership.GetUser("admin") == null)
                {
                    ...
                    admin = Membership.CreateUser("admin", "admin123%", "admin@DatatecnixOfficerII.com", "ASP.NET", "Code OnTime", true, out status);
                    user = Membership.CreateUser("user", "user123%", "user@DatatecnixOfficerII.com", "ASP.NET", "Code OnTime", true, out status);
                    offline = Membership.CreateUser("offline", "offline123%", "offline@DatatecnixOfficerII.com", "ASP.NET", "Code OnTime", true, out status);
        ...
        public virtual JObject OAuthReadUserClaims()
        {
            JObject claims = null;
            var userObj = TextUtility.ParseYamlOrJson(ApplicationServicesBase.Create().AppDataReadAllText(string.Format("sys/users/{0}.json", HttpContext.Current.User.Identity.Name)));
            if (userObj != null)
            {
                var idToken = Convert.ToString(userObj["id_token"]).Split('.');
                if (idToken.Length == 3)
                    claims = TextUtility.ParseYamlOrJson(Encoding.UTF8.GetString(TextUtility.FromBase64UrlEncoded(idToken[1])));
            }
            return claims;
        }
    ...
    public class ApplicationSessionStateBase : SessionStateStoreProviderBase
    {
        ...
        public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
        {
            ...
            if (newItem)
            {
                ...
                using (var cmd = CreateSqlText("INSERT INTO aspnet_sessions (session_id, application_name, created, expires, lock_date, lock_id, timeout, locked, session_items, flags) Values(@SessionId, @ApplicationName, @Created, @Expires, @LockDate, @LockId , @Timeout, @Locked, @SessionItems, @Flags)"))
                {
            ...
            {
                using (var cmd = CreateSqlText("UPDATE aspnet_sessions SET expires = @Expires, session_items = @SessionItems, locked = @Locked WHERE session_id = @SessionId AND application_name = @ApplicationName AND lock_id = @LockId"))
                {
                    cmd.AddParameter("SessionId", id);
                    cmd.AddParameter("Expires", DateTime.Now.AddMinutes(((double)(item.Timeout))));
                    cmd.AddParameter("SessionItems", sessItems);
        ...
        private SessionStateStoreData GetSessionStoreItem(bool lockRecord, HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actionFlags)
        {
            ...
            try
            {
                ...
                if (lockRecord)
                {
                    using (var cmd = CreateSqlText("UPDATE aspnet_sessions SET locked = @LockedTrue, lock_date = @LockDate WHERE session_id = @SessionId AND application_name = @ApplicationName AND locked = @LockedFalse AND expires > @Expires"))
                    {
                ...
                using (var cmd = CreateSqlText("SELECT expires, session_items, lock_id, lock_date, flags, timeout FROM aspnet_sessions WHERE session_id = @SessionId AND application_name = @ApplicationName"))
                {
                    cmd.AddParameter("SessionId", id);
                ...
                if (deleteData)
                {
                    using (var cmd = CreateSqlText("DELETE FROM aspnet_sessions WHERE session_id = @SessionId AND application_name = @ApplicationName"))
                    {
                        cmd.AddParameter("SessionId", id);
                        cmd.ExecuteNonQuery();
                    }
                ...
                {
                    lockId = (((int)(lockId)) + 1);
                    using (var cmd = CreateSqlText("UPDATE aspnet_sessions SET lock_id = @LockId, flags = 0 WHERE session_id = @SessionId AND application_name = @ApplicationName"))
                    {
                        cmd.AddParameter("lockId", lockId);
                        cmd.AddParameter("SessionId", id);
                        cmd.ExecuteNonQuery();
        ...
        public SqlText CreateSqlText(string sql)
        {
            ...
            cmd.Name = "DatatecnixOfficerII Session State Provider";
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_sessions SET locked = 0, expires = @Expires WHERE session_id = @SessionId AND application_name = @ApplicationName AND lock_id = @LockId"))
            {
        ...
        {
            using (var cmd = CreateSqlText("DELETE FROM aspnet_sessions WHERE session_id = @SessionId AND application_name = @ApplicationName AND lock_id = @LockId"))
            {
                cmd.AddParameter("SessionId", id);
                cmd.AddParameter("LockId", lockId);
                cmd.ExecuteNonQuery();
        ...
        {
            using (var cmd = CreateSqlText("INSERT INTO aspnet_sessions (session_id, application_name, created, expires, lock_date, lock_id, timeout, locked, flags) Values(@SessionId, @ApplicationName, @Created, @Expires, @LockDate, @LockId, @Timeout, @Locked, @Flags)"))
            {
                cmd.AddParameter("SessionId", id);
                cmd.AddParameter("Created", DateTime.Now);
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_sessions SET expires = @Expires WHERE session_id = @SessionId AND application_name = @ApplicationName"))
            {
                cmd.AddParameter("Expires", DateTime.Now.AddMinutes(_config.Timeout.TotalMinutes));
                cmd.AddParameter("SessionId", id);
                cmd.ExecuteNonQuery();
