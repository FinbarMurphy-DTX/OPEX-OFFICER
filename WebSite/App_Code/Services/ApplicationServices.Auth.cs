...
namespace DatatecnixOfficerII.Services
{
    ...
    public abstract class OAuthHandler
    {
        ...
        public virtual void ProcessRequest(HttpContext context)
        {
            ...
            catch (WebException ex)
            {
                ...
                else
                {
                    ...
                    context.Response.Write(string.Format("{0} {1}", ex.Status.ToString(), ex.Message));
        ...
        public virtual MembershipUser SyncUser()
        {
            ...
            if (string.IsNullOrEmpty(username))
                throw new Exception("Unable to identify the user with the information provided in thre response by the identity provider.");
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(url))
                    {
                        ...
                        using (var s = response.GetResponseStream())
                            avatar = ((Bitmap)(Bitmap.FromStream(s)));
                        using (var ms = new MemoryStream())
                        {
                            avatar.Save(ms, ImageFormat.Png);
                            // delete existing avatars
                            foreach (var file in ApplicationServicesBase.Create().ReadSiteContent("sys/users", (user.UserName + ".%")))
                                if (file.ContentType.StartsWith("image/"))
                                    SiteContentFile.Delete(file.FullName);
                            // save the new avatar
    ...
    public class OAuthHandlerFactoryBase
    {
        ...
        {
            var oauth2 = Regex.Match(service, "^oauth2(\\-(.+))?$", RegexOptions.IgnoreCase);
            if (oauth2.Success)
                service = "oauth2";
            Type t = null;
            if (Handlers.TryGetValue(service.ToLower(), out t))
            {
                var handler = ((OAuthHandler)(Activator.CreateInstance(t)));
                if (oauth2.Success)
                    ((OAuth2Handler)(handler)).Name = oauth2.Groups[2].Value;
                return handler;
            }
            return null;
        }
    ...
    public partial class AppIdentityOAuthHandlerBase : OAuthHandler
    {
        ...
        public override MembershipUser SyncUser()
        {
            ...
            if (ApplicationServicesBase.IsSiteContentEnabled && ((user != null) && (Tokens != null)))
                SiteContentFile.WriteJson(string.Format("sys/users/{0}.json", user.UserName), Tokens);
            return user;
        }
        ...
        public override bool ValidateRefreshToken(MembershipUser user, string token)
        {
            if (!ApplicationServices.IsSiteContentEnabled)
                return true;
            Tokens = SiteContentFile.ReadJson(string.Format("sys/users/{0}.json", user.UserName));
            var pictureUpdated = Tokens["picture_updated"];
            var tokenRequestParams = new JObject();
            tokenRequestParams["grant_type"] = "refresh_token";
            tokenRequestParams["refresh_token"] = Tokens["refresh_token"];
            tokenRequestParams["client_id"] = Config.ClientId;
            tokenRequestParams["client_secret"] = Config.ClientSecret;
            var authRequestData = Encoding.UTF8.GetBytes(tokenRequestParams.ToString());
            // refresh tokens at 'oauth2/v2/token'
            var providerUri = new Uri(new Uri(ClientUri), "oauth2/v2/token");
            try
            {
                var authRequest = WebRequest.Create(providerUri);
                authRequest.Method = "POST";
