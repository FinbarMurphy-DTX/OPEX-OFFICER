...
using DatatecnixOfficerII.Security;
...
namespace DatatecnixOfficerII.Services
{
        ...
        protected virtual void CombineScripts(HttpContext context, bool caching, bool isSaaS, string culture)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$('<link></link>').appendTo($('head')).attr({{ href: '{0}/App_Themes/DatatecnixOfficerII/_Theme_Aquarium.css?{0}', type: 'text/css', rel: 'stylesheet' }});", ApplicationServices.Version), baseUrl);
                try
                {
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("DatatecnixOfficerII=typeof DatatecnixOfficerII=='undefined'?{{}}:DatatecnixOfficerII;DatatecnixOfficerII.{0}=", instance);
                }
            }
        }
    ...
    public partial class OAuth2Handler : OAuth2HandlerBase
    {
    }

    public partial class OAuth2HandlerBase : OAuthHandler
    {

        private JObject _identityRequest;
        ...
        public override string GetAuthorizationUrl()
        {
            var state = TextUtility.GetUniqueKey(16);
            var redirectUri = BuildRedirectUri();
            var scope = new List<string>(Regex.Split(Config["Scope"], "\\s+"));
            ...
            if (Config["Grant Type"] == "authorization_code_with_pkce")
            {
                codeChallenge = Config["Code Verifier"];
                if (string.IsNullOrEmpty(codeChallenge))
                    codeChallenge = TextUtility.GetUniqueKey(64, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890-._~");
                codeVerifier = codeChallenge;
                codeChallengeMethod = Config["Code Challenge Method"];
            ...
            var identityRequest = new JObject();
            identityRequest["grant_type"] = "authorization_code";
            identityRequest["client_id"] = Config.ClientId;
            identityRequest["state"] = state;
            identityRequest["redirect_uri"] = redirectUri;
            identityRequest["scope"] = string.Join(" ", scope.ToArray()).Trim();
            identityRequest["code_verifier"] = codeVerifier;
            PersistIdentityRequest(identityRequest);
            // build an authorization URL
            var uriBuilder = new UriBuilder(Config["Auth Url"]);
            var query = new List<string>();
            if (!string.IsNullOrEmpty(uriBuilder.Query))
                query.Add(uriBuilder.Query.Substring(1));
            query.Add("response_type=code");
            query.Add(("client_id=" + Uri.EscapeDataString(Config.ClientId)));
            query.Add(("redirect_uri=" + Uri.EscapeDataString(redirectUri)));
            query.Add(("scope=" + Uri.EscapeDataString(((string)(identityRequest["scope"])))));
            ...
            var authUrlParameters = Config.ToDictionary("Auth Url Parameters");
            foreach (var paramName in authUrlParameters.Keys)
                query.Add(string.Format("{0}={1}", paramName, Uri.EscapeDataString(authUrlParameters[paramName])));
            uriBuilder.Query = string.Join("&", query.ToArray());
        ...
        public virtual void PersistIdentityRequest(JObject identityRequest)
        {
            ...
            foreach (var filename in app.AppDataSearch("sys/oauth2/generic", "%.json", 5, DateTime.UtcNow.AddMinutes(-30)))
                app.AppDataDelete(filename);
            app.AppDataWriteAllText(string.Format("sys/oauth2/generic/{0}.json", identityRequest["state"]), identityRequest.ToString());
        }

        public override void SetState(string state)
        {
            // restore the pending "authorize-server" request
            var requestPath = string.Format("sys/oauth2/generic/{0}.json", state);
            _identityRequest = TextUtility.ParseYamlOrJson(ApplicationServicesBase.Create().AppDataReadAllText(requestPath));
            if (_identityRequest != null)
                StartPage = ((string)(_identityRequest["start_page"]));
            else
                _identityRequest = new JObject();
        }

        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            var request = WebRequest.Create(Config["Access Token Url"]);
            request.Method = "POST";
            var accessTokenUrlHeaders = Config.ToDictionary("Access Token Url Headers");
            foreach (var paramName in accessTokenUrlHeaders.Keys)
                request.Headers[paramName] = accessTokenUrlHeaders[paramName];
            var codeType = "code";
            if (refresh)
                codeType = "access_token";
            var query = new List<string>();
            query.Add(("grant_type=" + Convert.ToString(_identityRequest["grant_type"])));
            query.Add(string.Format("{0}={1}", codeType, code));
            AddCredentials(request, query);
            query.Add(("redirect_uri=" + Uri.EscapeDataString(((string)(_identityRequest["redirect_uri"])))));
            var codeVerifier = Convert.ToString(_identityRequest["code_verifier"]);
            if (!string.IsNullOrEmpty(codeVerifier))
                query.Add(("code_verifier=" + Uri.EscapeDataString(codeVerifier)));
            var accessTokenUrlBody = Config.ToDictionary("Access Token Url Body");
            foreach (var paramName in accessTokenUrlBody.Keys)
                query.Add(string.Format("{0}={1}", paramName, Uri.EscapeDataString(accessTokenUrlBody[paramName])));
            var body = string.Join("&", query.ToArray());
            var bodyBytes = Encoding.UTF8.GetBytes(body);
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = bodyBytes.Length;
            using (var stream = request.GetRequestStream())
        ...
        public override JObject Query(string method, bool useSystemToken)
        {
            ...
            ApplicationServicesBase.Create().OAuthSetUserObject(claims);
            return claims;
        }
        ...
        public override string GetUserName()
        {
            ...
            var userName = Convert.ToString(_claims.SelectToken("preferred_username"));
            if (string.IsNullOrEmpty(userName))
                userName = Convert.ToString(_claims.SelectToken("email"));
            return userName;
        }

        public override string GetUserEmail()
        {
            return Convert.ToString(_claims.SelectToken("email"));
        }
        ...
        public override string GetUserImageUrl(MembershipUser user)
        {
            if (Tokens != null)
            {
                ...
                if ((pictureUpdated == null) || (TextUtility.ToUniversalTime(pictureUpdated).AddMinutes(PictureUpdateFrequency) < DateTime.UtcNow))
                {
                    ...
                    return Convert.ToString(_claims.SelectToken("picture"));
                }
            }
            return null;
        }
        ...
        public override MembershipUser SyncUser()
        {
            ...
            if (ApplicationServicesBase.IsSiteContentEnabled && ((user != null) && (Tokens != null)))
            {
                if (!Tokens.ContainsKey("expires_datetime"))
                {
                    var expiresIn = Convert.ToInt32(Tokens["expires_in"]);
                    if (expiresIn <= 0)
                        expiresIn = 300;
                    Tokens["expires_datetime"] = DateTime.UtcNow.AddSeconds(expiresIn).ToString("O");
                }
                var app = ApplicationServicesBase.Create();
                app.AppDataWriteAllText(string.Format("sys/users/{0}.json", user.UserName), Tokens.ToString());
                if (_identityRequest != null)
                    app.AppDataDelete(string.Format("sys/oauth2/generic/{0}.json", _identityRequest["state"]));
            }
            return user;
        }
        ...
        public override void SignOut()
        {
            ...
            var app = ApplicationServicesBase.Create();
            Tokens = TextUtility.ParseYamlOrJson(app.AppDataReadAllText(userTokensFileName));
            if (Tokens != null)
            {
                app.AppDataDelete(userTokensFileName);
                var revokeUrl = Config["Revoke Url"];
                if (!string.IsNullOrEmpty(revokeUrl) && (Tokens["access_token"] != null))
                    try
                    {
                        var revokeRequest = WebRequest.Create(revokeUrl);
                        revokeRequest.Method = "POST";
                        var query = new List<string>();
                        query.Add(("token=" + Uri.EscapeDataString(((string)(Tokens["access_token"])))));
                        AddCredentials(revokeRequest, query);
                        var body = string.Join("&", query.ToArray());
                        var bodyBytes = Encoding.UTF8.GetBytes(body);
                        revokeRequest.ContentType = "application/x-www-form-urlencoded";
                        revokeRequest.ContentLength = bodyBytes.Length;
        ...
        public override bool ValidateRefreshToken(MembershipUser user, string token)
        {
            if (!ApplicationServicesBase.IsSiteContentEnabled)
                return true;
            Tokens = TextUtility.ParseYamlOrJson(ApplicationServicesBase.Create().AppDataReadAllText(string.Format("sys/users/{0}.json", user.UserName)));
            if ((Tokens == null) || (Tokens["access_token"] == null))
                return true;
            // make sure that the access_token is still valid
            var expiresDateTime = Convert.ToString(Tokens["expires_datetime"]);
            if (!string.IsNullOrEmpty(expiresDateTime))
            {
                if (DateTime.UtcNow < TextUtility.ToUniversalTime(expiresDateTime))
                    return true;
            }
            var refreshToken = Convert.ToString(Tokens["refresh_token"]);
            if (string.IsNullOrEmpty(refreshToken))
                return false;
            // contact the authorizaton server to refresh the access_token
            try
            {
                var pictureUpdated = Tokens["picture_updated"];
                var authRequest = WebRequest.Create(Config["Access Token Url"]);
                authRequest.Method = "POST";
                var accessTokenUrlRefreshHeaders = Config.ToDictionary("Access Token Url Refresh Headers");
                foreach (var paramName in accessTokenUrlRefreshHeaders.Keys)
                    authRequest.Headers[paramName] = accessTokenUrlRefreshHeaders[paramName];
                var query = new List<string>();
                query.Add("grant_type=refresh_token");
                query.Add(("refresh_token=" + Uri.EscapeDataString(refreshToken)));
                AddCredentials(authRequest, query);
                var accessTokenUrlRefreshBody = Config.ToDictionary("Access Token Url Refresh Body");
                foreach (var paramName in accessTokenUrlRefreshBody.Keys)
                    query.Add(string.Format("{0}={1}", paramName, Uri.EscapeDataString(accessTokenUrlRefreshBody[paramName])));
                var body = string.Join("&", query.ToArray());
                var bodyBytes = Encoding.UTF8.GetBytes(body);
                ...
                using (var stream = authRequest.GetRequestStream())
                {
                    ...
                    using (var sr = new StreamReader(response.GetResponseStream()))
                        Tokens = TextUtility.ParseYamlOrJson(sr.ReadToEnd());
                    if (Tokens["access_token"] == null)
                        return false;
                    if (pictureUpdated != null)
                        Tokens["picture_updated"] = pictureUpdated;
                }
                SyncUser();
                return true;
            }
            catch (WebException ex)
            {
                if (ex.Response != null)
                {
                    using (var stream = ex.Response.GetResponseStream())
                    {
                        using (var reader = new StreamReader(stream))
                        {
                            // inspect the refresh error text
                            var text = reader.ReadToEnd();
                        }
                    }
                }
        ...
        protected virtual void AddCredentials(WebRequest request, List<string> query)
        {
            if (Config["Client Authentication"] == "header")
                request.Headers["Authorization"] = ("Basic " + Convert.ToBase64String(Encoding.UTF8.GetBytes(string.Format("{0}:{1}", Config.ClientId, Config.ClientSecret))));
            else
            {
                query.Add(("client_id=" + Uri.EscapeDataString(Config.ClientId)));
                if (!string.IsNullOrEmpty(Config.ClientSecret))
                    query.Add(("client_secret=" + Uri.EscapeDataString(Config.ClientSecret)));
            }
        }
    }
}
