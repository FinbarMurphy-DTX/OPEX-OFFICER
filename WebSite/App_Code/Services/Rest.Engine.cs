...
namespace DatatecnixOfficerII.Services.Rest
{
    ...
    public partial class RESTfulResourceBase : RESTfulResourceConfiguration
    {
            ...
            foreach (var df in fieldList)
                if (!ignoreList.Contains(df))
                {
                    ...
                    if (((links != null) && !string.IsNullOrEmpty(df.ItemsDataController)) && (((df.ItemsDataController == ControllerName) && (PK.Count == 1)) && ((Action == null) && AllowController(df.ItemsDataController))))
                    {
                        ...
                        ExtendLinkWith("embeddable", true, AddLink(ChildrenKey, ToApiFieldName(df.Name), "GET", links, ExtendRawUrlWith(false, (string.Join("/", childrenPath) + "?count=true"))));
                    }
                    if (addValueProperty)
        ...
        public void CreateLookupLink(DataField field, JProperty links)
        {
            if ((Action == null) && Convert.ToBoolean(ApplicationServicesBase.SettingsProperty("server.rest.hypermedia.links.lookups", true)))
            {
                ...
                if ((field.ItemsDataController == ControllerName) && (IsCollection && ((LookupNavigationIdentifier(ControllerName) == null) && !Location.EndsWith(string.Format("/{0}/{1}", RootKey, ToPathName(field.Name))))))
                    ExtendLinkWith("embeddable", true, AddLink(RootKey, ToApiFieldName(field.Name), "GET", links, ExtendRawUrlWith("/{0}/{1}?count=true", RootKey, ToPathName(field.Name))));
            }
        }
