        ...
        protected override void VirtualizeController(string controllerName)
        {
            ...
            if (OAuthProviders.Count > 0)
            {
                ...
                if (items.Count > 0)
                {
                    ...
                    var displayNames = new SortedDictionary<string, string>();
                    foreach (var file in OAuthProviders)
                    {
                        ...
                        try
                        {
                            ...
                            var displayName = Convert.ToString(providerDef["Display Name"]);
                            if (!string.IsNullOrEmpty(displayName))
                                displayNames[file.Name] = displayName;
                    ...
                    // create a list of "named" OAuth2 providers
                    var oauth2Providers = new SortedDictionary<string, SiteContentFile>();
                    foreach (var p in OAuthProviders)
                        if (p.Name.StartsWith("oauth2-"))
                        {
                            var displayName = Path.GetFileNameWithoutExtension(p.Name);
                            if (displayNames.ContainsKey(displayName))
                                displayName = displayNames[displayName];
                            oauth2Providers[displayName] = p;
                        }
                    foreach (var displayName in oauth2Providers.Keys)
                    {
                        var provider = oauth2Providers[displayName];
                        items[0].InsertBefore(string.Format("<item value=\"{0}\"/>", Path.GetFileNameWithoutExtension(provider.Name)));
                        var newItem = items[0].SelectSingleNode("preceding-sibling::*[1]");
                        newItem.CreateAttribute(string.Empty, "text", string.Empty, displayName);
                    }
                    // trim the list of the standard providers
                    foreach (var item in items)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (displayNames.ContainsKey(provider))
                                    otherItemText.SetValue(displayNames[provider]);
                                else
                                {
