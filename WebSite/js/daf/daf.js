...
(function () {
    ...
    _app.prototype = {
        ...
        hasDetails: function () {
            ...
            return !/*that.odp*/that.allowDetails() && !!result;
        },
        allowDetails: function () {
            var result = false;
            if (this.odp) {
                result = true;
                for (var i = 0; i < this._selectedKey.length; i++)
                    if (this._selectedKey[i] == null)
                        result = false;
            }
            return result;
        },
            ...
            if (!expression._variables) {
                ...
                while (m) {
                    ...
                    if (!found) {
                        var f = this.findField(fieldName);
                        if (f)
                            Array.add(vars, { 'name': fieldName, 'regex': new RegExp('(\\[' + fieldName + '\\])|((\\$row|this)\\.' + fieldName + '\\b)', 'g'), 'replace': String.format('this._javaScriptRowValue({0})', f.Index) });
                    }
                    m = re.exec(expression.Test);
                }
