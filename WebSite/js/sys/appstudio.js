...
(function () {
        ...
        inspect: function (x, y) {
            ...
            if (inspectorConfig) {
                ...
                if (typeof navigate == 'function')
                    navigate = navigate.call(selector, selector);
    ...
    $(document).on('vclick', '.studio #studio .ui-content', function (e) {
        ...
        return false;
    }).on('studioactionpages.app', e => {
        if (!isStudio) {
            appStudioStart({ event: { type: 'studioactionpages.app' } });
        }
        else {
            //var objectType = e?.treeView?.node.type;
            getStudioTools()
                ...
                .then(() => {
                    $app.touch.propGrid('show', {
                        instance: 'studio.pages',
                        text: 'Pages',
                        icon: 'material-icon-menu-book',
                        nodes: fetchHierarchy(`/v2/studio/hierarchies/pages/definition`),
                        helpUrl: 'https://codeontime.com/roadmap/v9',
                        location: 'left'
                    });
                    notifyIncompleteImplementation('pages');
                })
                .catch(ex => studioError(ex));
        ...
        return false;
    }).on('studioaction222publish.app', e => {
        if (!isStudio)
            appStudioStart({ event: { type: 'studioactionpublish' } });
        else {
            _app.touch.show({
                text: 'Publish',
                text2: 'App Studio',
                ...
                topics: [
                    {
                        ...
                        questions: [
                            {
                                name: 'OutputPath',
                                placeholder: 'An alternative location for the published files'
                            },
                            {
                                name: 'Frontend',
                                required: true,
                    ...
                    {
                        wrap: true,
                        text: 'Miscellaneous (Delete this)',
                        questions: [
                            {
                                text: false,
    ...
    $(document)
        .on('treeview.app', e => {
            if (isStudio) {
                ...
                if (treeView.eventName === 'get') {
                    e.preventDefault();
                    if (treeView.nodeData)
                        treeView.result = studioRestful({
                            url: treeView.nodeData._links.self
                        });
                }
            }
        })
    ...
    function fetchFromProject(options) {
        ...
        if (!hypermedia && !context)
            return Promise.resolve(null);
        if (typeof hypermedia == 'object' && hypermedia.hypermedia) {
            parameters = hypermedia;
        ...
        if (parameters)
            for (var key in parameters)
                if (!(key.match(/^hypermedia$/)))
                    request[key] = parameters[key];
        return studioRestful(request).then(response => {
    ...
    function studioRestful(options) {
        ...
        var url = options.url;
        if (url && typeof url != 'string' && url.method !== 'GET') {
            var segments = url.href.split(/\//g);
            if (segments.length > 5)
                options.cacheTrimDepth = segments.length - 3;
